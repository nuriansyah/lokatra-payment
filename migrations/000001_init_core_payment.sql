...
-- =============================================================================
CREATE TYPE payment_provider_type_enum AS ENUM ('gateway','aggregator','acquirer','bank','wallet','cash');
CREATE TYPE payment_provider_status_enum AS ENUM ('active', 'inactive', 'deprecated');
...
CREATE TYPE payment_intent_status_enum AS ENUM ('requires_payment_method', 'requires_confirmation', 'requires_action', 'processing', 'succeeded', 'canceled','failed','expired','reconciliation_required', 'manual_review_required', 'manual_review_approved', 'manual_review_rejected','overpaid','refunded', 'partially_refunded','unknown');
...
CREATE TABLE IF NOT EXISTS payment_account_capabilities (
  ...
  meta_deleted_at timestamptz NULL,
  meta_deleted_by uuid NULL,
  UNIQUE(provider_account_id, payment_method_id, payment_channel_id)
);